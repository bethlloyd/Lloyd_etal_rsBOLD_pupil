...
# indicate which figure to work on 
which_figure <- 'Fig_5' #  Fig_S4

if (which_figure == 'Fig_5') {
  data_path <- "D:\\NYU_RS_LC\\stats\\OSF_repository\\Figure5-S3-SourceData"
  type = 'size'
} else if (which_figure == 'Fig_S4') {
  data_path <- "D:\\NYU_RS_LC\\stats\\OSF_repository\\FigureS4-SourceData" 
  type = 'deriv'
}
...
for (c_roi in rois){
  ...
  day1 <- read.delim(paste(data_path, 'ses-day-1', paste("XCorr_stat_", c_roi, '_pup_', type, '.csv',  sep = ''), 
                                  sep = '\\'), header = TRUE, sep = ',')
  names(day1)[names(day1) == 'CC'] <- 'CC_D1'
  day2 <- read.delim(paste(data_path, 'ses-day-2', paste("XCorr_stat_", c_roi, '_pup_', type,'.csv',  sep = ''), 
                           sep = '\\'), header = TRUE, sep = ',')
  ...
  for (c_mod in P_mods) { 
    ...
    stat.test_p1 <- 
      t.test(dat_model$CC_both,
             mu = 0) # Remove details
    print(paste('is the uncorrected p-value for Z corr at lag ', c_mod, t_apa(stat.test_p1), sep =" "))
...
# correct for multple comparisons (FDR-correction) on t-tests for each time bin
options(scipen=999)
